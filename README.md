...
Мотивация
------------
Для отслеживания текущего состояния приложений используется программа `monitor` <https://monitor.rg.ru>.
Однако этого недостаточно для понимания того, что происходит или происходило в системе в прошлом.

**log-monitor** предназначен для сбора логов приложений
и их анализа в режиме почти реального времени. 
"Почти" означает, что данные поступают с запозданием в несколько секунд требующихся для их индексации.
...
Как настроить приложение для посылки логов в Эластик
-----------------------------------------------


Как находить логи приложения
-------------------------------
...
log-generator также может отправлять новые записи лога напрямую в Эластик в
